...
void render_prerender_coord_textures(GameState *gs) {
    ...
    for (int r = 0; r < gs->map_rows; r++) for (int c = 0; c < gs->map_cols; c++) {
...
int render_set_info_lines(GameState *gs, const char **lines, int nlines) {
    ...
    gs->info_w=dest->w; gs->info_h=dest->h; SDL_FreeSurface(dest);
...
void render_show_sprite_info_ex(GameState *gs, Sprite *s, UnitRole role, Faction faction) {
    ...
    make_bar(hp_bar,sizeof(hp_bar),s->hp,s->max_hp,12); snprintf(l_hp,sizeof(l_hp),"HP  %s",hp_bar);
    make_bar(mp_bar,sizeof(mp_bar),s->mp,s->max_mp,12); snprintf(l_mp,sizeof(l_mp),"MP  %s",mp_bar);
    ...
    const char *lines[]={l_header,l_sep,l_job,l_level,l_faction,l_pos,l_sep,
                         l_hp,l_mp,l_status,l_sep,l_atk,l_def,l_spd,l_mov,l_sep,
...
void render_hex_terrain(SDL_Renderer *renderer, int cx, int cy, int radius, Terrain t) {
    ...
    SDL_RenderDrawLines(renderer,pts,6); SDL_RenderDrawLine(renderer,pts[5].x,pts[5].y,pts[0].x,pts[0].y);
}
...
static void pass_terrain(GameState *gs) {
    ...
    for(int row=0;row<gs->map_rows;row++) for(int col=0;col<gs->map_cols;col++){
        ...
        if(gs->show_cell_coords_enabled&&rad>=COORDS_SHOW_MIN_RADIUS&&gs->coord_textures){
            ...
            if(ttx){int tw,th;SDL_QueryTexture(ttx,NULL,NULL,&tw,&th);
                    SDL_SetTextureBlendMode(ttx,SDL_BLENDMODE_BLEND);
                    SDL_Rect td={cx-tw/2,cy-th/2,tw,th};SDL_RenderCopy(rdr,ttx,NULL,&td);}
        }
    }
}
...
static void pass_selection(GameState *gs) {
    ...
    int cx,cy; hex_center(gs->selected_row,gs->selected_col,gs->current_radius,
...
static void pass_neighbors(GameState *gs) {
    ...
    for(int i=0;i<nc;++i){
        ...
        SDL_RenderDrawLines(rdr,pts,6); SDL_RenderDrawLine(rdr,pts[5].x,pts[5].y,pts[0].x,pts[0].y);
    }
...
static void pass_path_line(GameState *gs) {
    ...
    for(int i=0;i<path_len;++i){
        int pr=path_nodes[i]/gs->map_cols, pc=path_nodes[i]%gs->map_cols;
        hex_center(pr,pc,gs->current_radius,gs->camera.x,gs->camera.y,&pline[i].x,&pline[i].y);
    }
    SDL_SetRenderDrawBlendMode(gs->renderer,SDL_BLENDMODE_BLEND);
...
static void pass_units(GameState *gs) {
    ...
    for(int i=0;i<gs->units.count;i++){
        ...
        /* Choose source rect based on role + animation state */
        SDL_Rect src;
        int is_moving_this = (mv->moving && mv->unit_idx == i);
        if (u->atlas_run_idx >= 0 && is_moving_this) {
            /* Use this unit's assigned run animation */
            const AtlasSprite *rs = &gs->atlas.sprites[u->atlas_run_idx];
            int frame_w = rs->src.w;
            src.x = rs->src.x + mv->run_frame * frame_w;
            src.y = rs->src.y;
            src.w = frame_w;
            src.h = rs->src.h;
        } else if (u->atlas_idle_idx >= 0) {
            /* Use this unit's assigned idle animation */
            src = gs->atlas.sprites[u->atlas_idle_idx].src;
        } else {
            /* Fallback: legacy role_src / idle_src */
            if (u->role == UNIT_ROLE_PLAYER) {
                if (is_moving_this && gs->atlas.run_frames > 0 && gs->atlas.run_w > 0) {
            ...
            } else if (u->role == UNIT_ROLE_ENEMY) {
                if (is_moving_this && gs->atlas.enemy_run_frames > 0 && gs->atlas.enemy_run_w > 0) {
                    src.x = gs->atlas.enemy_run_src.x + mv->run_frame * gs->atlas.enemy_run_w;
                    src.y = gs->atlas.enemy_run_src.y;
                    src.w = gs->atlas.enemy_run_w;
                    src.h = gs->atlas.enemy_run_h;
                } else {
                    src = (gs->atlas.role_src[UNIT_ROLE_ENEMY].w > 0)
                        ? gs->atlas.role_src[UNIT_ROLE_ENEMY] : gs->atlas.idle_src;
                }
             } else {
                src = (gs->atlas.role_src[u->role].w > 0) ? gs->atlas.role_src[u->role] : gs->atlas.idle_src;
             }
        }
        ...
        else if(u->role==UNIT_ROLE_PLAYER&&gs->turn.player_moved&&gs->turn.player_attacked)
...
void render_action_menu(GameState *gs) {
    ...
    static const char *labels[MENU_ITEM_COUNT]={"  Move  "," Attack ","  Skill ","  Item  ","  Wait  "};
    ...
    for(int i=0;i<MENU_ITEM_COUNT;i++){
        ...
        if(disabled)      SDL_SetRenderDrawColor(rdr,40,40,40,180);
        else if(i==m->hovered) SDL_SetRenderDrawColor(rdr,255,220,60,230);
        else              SDL_SetRenderDrawColor(rdr,30,30,50,210);
        SDL_RenderFillRect(rdr,&item);
        SDL_SetRenderDrawColor(rdr,disabled?80:160,disabled?80:160,disabled?80:200,disabled?180:255);
        SDL_RenderDrawRect(rdr,&item);
        SDL_Color col;
        if(disabled)           col=(SDL_Color){100,100,100,200};
        else if(i==m->hovered) col=(SDL_Color){20,20,20,255};
        else                   col=(SDL_Color){220,220,220,255};
        SDL_Surface *surf=TTF_RenderUTF8_Blended(gs->font,labels[i],col);
        if(surf){
            SDL_Texture *tex=SDL_CreateTextureFromSurface(rdr,surf);
            if(tex){SDL_Rect dst={item.x+(item.w-surf->w)/2,item.y+(item.h-surf->h)/2,surf->w,surf->h};
                    SDL_RenderCopy(rdr,tex,NULL,&dst);SDL_DestroyTexture(tex);}
            SDL_FreeSurface(surf);
        }
    }
}
