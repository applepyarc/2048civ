...
static void generate_terrain(GameState *gs) {
    ...
    for (int r=0;r<gs->map_rows;r++) for (int c=0;c<gs->map_cols;c++) {
        float elev=perlin_elevation((float)c,(float)r), mois=perlin_moisture((float)c,(float)r);
        Terrain t;
...
static void load_atlas(GameState *gs) {
    if (!(IMG_Init(IMG_INIT_PNG)&IMG_INIT_PNG)) {fprintf(stderr,"SDL_image PNG unavailable: %s\n",IMG_GetError());return;}
    const char *atlas_img=config_get_atlas_image_path(), *atlas_desc=config_get_atlas_desc_path();
    SDL_Surface *surf=IMG_Load(atlas_img);
    if (!surf) {fprintf(stderr,"Atlas load failed: %s\n",IMG_GetError());return;}
    gs->atlas.tex=SDL_CreateTextureFromSurface(gs->renderer,surf); SDL_FreeSurface(surf);
    if (!gs->atlas.tex) return;
    gs->atlas.idle_src=(SDL_Rect){0,0,16,16}; gs->atlas.run_src=(SDL_Rect){0,0,16,16};
    gs->atlas.run_frames=0; gs->atlas.run_w=gs->atlas.run_h=16;
    ...
    while (fscanf(af,"%255s",name)==1) {
        ...
        fscanf(af,"%d",&frames);
        if (!strcmp(name,"knight_f_idle_anim")) {gs->atlas.idle_src=(SDL_Rect){x,y,w,h};gs->atlas.role_src[UNIT_ROLE_PLAYER]=gs->atlas.idle_src;}
        else if (!strcmp(name,"knight_f_run_anim")) {gs->atlas.run_src=(SDL_Rect){x,y,w,h};gs->atlas.run_frames=frames>0?frames:1;gs->atlas.run_w=w;gs->atlas.run_h=h;}
        else if (!strcmp(name,"big_zombie_idle_anim")) gs->atlas.role_src[UNIT_ROLE_ENEMY]=(SDL_Rect){x,y,w,h};
    }
    fclose(af);
}
...
static void populate_units(GameState *gs) {
    Unit *player=um_add(&gs->units,"Player","Warrior",1,UNIT_ROLE_PLAYER,FACTION_PLAYER,rand()%gs->map_rows,rand()%gs->map_cols);
    if (player&&player->sprite) {
        ...
        printf("Player @ (%d,%d) lvl=%d HP=%d ATK=%d\n",p->x,p->y,p->level,p->hp,p->attack);
    }
    for (int i=0;i<2;i++) {
        char ename[32]; snprintf(ename,sizeof(ename),"Enemy%d",i+1);
        Unit *enemy=um_add(&gs->units,ename,"Goblin",1+rand()%5,UNIT_ROLE_ENEMY,FACTION_ENEMY,rand()%gs->map_rows,rand()%gs->map_cols);
        if (enemy&&enemy->sprite) {
            Sprite *e=enemy->sprite;
            e->max_hp=40+rand()%81;e->hp=e->max_hp;e->attack=4+rand()%20;e->defense=rand()%15;e->speed=1+rand()%6;e->move=1+rand()%2;
            printf("Enemy  @ (%d,%d) lvl=%d HP=%d ATK=%d\n",e->x,e->y,e->level,e->hp,e->attack);
        }
    }
}
...
static int game_init(GameState *gs) {
    ...
    config_init();
    gs->window_width=config_get_window_width(); gs->window_height=config_get_window_height();
    float split=config_get_split_ratio(); if(split<0.05f)split=0.05f; if(split>0.95f)split=0.95f;
    gs->main_width=(int)(gs->window_width*split);
    gs->window=SDL_CreateWindow("2048civ",SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED,gs->window_width,gs->window_height,0);
    ...
    srand((unsigned)time(NULL)); generate_terrain(gs);
    gs->current_radius=DEFAULT_HEX_RADIUS; eh_compute_map_bounds(gs); eh_clamp_camera(gs);
    render_prerender_coord_textures(gs);
    um_init(&gs->units,16); populate_units(gs); load_atlas(gs);
    gs->move.move_ms=config_get_move_ms(); gs->move.anim_frame_ms=120; gs->move.unit_idx=-1;
    gs->move.from_r=gs->move.from_c=gs->move.to_r=gs->move.to_c=-1;
    tm_init(&gs->turn); tm_begin_turn(&gs->turn,&gs->units,FACTION_PLAYER);
    gs->highlight_neighbors_enabled=1; gs->show_cell_coords_enabled=1; gs->render_layers=RLAYER_ALL;
    ...
    gs->selected_row=gs->selected_col=gs->hover_row=gs->hover_col=-1;
    gs->phase=PHASE_FREE_ROAM; gs->running=1; return 1;
}

/* ---- Logic update ---- */
static void game_update(GameState *gs) {
    MoveState *mv=&gs->move;
    if (!mv->moving||path_len<=1||!path_nodes||mv->move_index>=path_len) return;
    Uint32 now=SDL_GetTicks();
    if (mv->move_ms<=0) mv->move_ms=200;
    mv->progress+=(float)(now-mv->last_move_tick)/(float)mv->move_ms;
    mv->last_move_tick=now;
