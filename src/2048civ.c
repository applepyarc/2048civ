...
static void load_atlas(GameState *gs) {
    ...
    for (int i = 0; i < 4; i++)
        gs->atlas.role_src[i] = (SDL_Rect){0, 0, 16, 16};
...
static void populate_units(GameState *gs) {
    ...
    if (player && player->sprite) {
        ...
        p->level   = 1 + rand() % 10;
        p->max_hp  = 80  + rand() % 121; p->hp = p->max_hp;
        p->max_mp  = 10  + rand() % 71;  p->mp = p->max_mp;
        p->attack  = 5   + rand() % 36;
        p->defense = rand() % 31;
        p->speed   = 1   + rand() % 10;
        p->move    = 1   + rand() % 3;
    ...
    for (int i = 0; i < 2; i++) {
        char ename[32];
        snprintf(ename, sizeof(ename), "Enemy%d", i + 1);
        Unit *enemy = um_add(&gs->units,
                             ename, "Goblin", 1 + rand() % 5,
                             UNIT_ROLE_ENEMY, FACTION_ENEMY,
                             rand() % gs->map_rows, rand() % gs->map_cols);
        if (enemy && enemy->sprite) {
            Sprite *e = enemy->sprite;
            e->max_hp  = 40 + rand() % 81; e->hp = e->max_hp;
            e->attack  = 4  + rand() % 20;
            e->defense = rand() % 15;
            e->speed   = 1  + rand() % 6;
            e->move    = 1  + rand() % 2;
...
static int game_init(GameState *gs) {
    ...
    if (split > 0.95f) split = 0.95f;
    gs->main_width = (int)(gs->window_width * split);

    gs->window = SDL_CreateWindow("2048civ",
    ...
    gs->map_rows    = config_get_map_rows();
    gs->map_cols    = config_get_map_cols();
    ...
    gs->move.move_ms       = config_get_move_ms();
    gs->move.anim_frame_ms = 120;
    gs->move.unit_idx      = -1;
    gs->move.from_r = gs->move.from_c = -1;
    gs->move.to_r   = gs->move.to_c   = -1;

    tm_init(&gs->turn);
    tm_begin_turn(&gs->turn, &gs->units, FACTION_PLAYER);
...
static void game_update(GameState *gs) {
    /* Enemy AI: drive movement animation each tick, then run the AI state machine */
    if (gs->turn.phase == TURN_PHASE_ENEMY_AI) {
        ...
        if (mv->moving && path_len > 1 && path_nodes && mv->move_index < path_len) {
            ...
            Uint32 dt = now - mv->last_move_tick;
            mv->last_move_tick = now;
            mv->progress += (float)dt / (float)mv->move_ms;

            while (mv->progress >= 1.0f && mv->moving) {
                Unit *u = (mv->unit_idx >= 0 && mv->unit_idx < gs->units.count)
                          ? &gs->units.units[mv->unit_idx] : NULL;
                if (u && u->sprite && mv->to_r >= 0 && mv->to_c >= 0)
                    sprite_set_position(u->sprite, mv->to_r, mv->to_c);
                ...
                if (mv->move_index < path_len) {
                    ...
                    int idx  = path_nodes[mv->move_index];
                    mv->to_r = idx / gs->map_cols;
                    mv->to_c = idx % gs->map_cols;
                } else {
                    mv->moving   = 0;
                    mv->progress = 0.0f;
                    mv->from_r = mv->from_c = mv->to_r = mv->to_c = -1;
            ...
            if (gs->atlas.run_frames > 0
                    && now - mv->last_anim_tick >= (Uint32)mv->anim_frame_ms) {
                mv->run_frame = (mv->run_frame + 1) % gs->atlas.run_frames;
                mv->last_anim_tick = now;
            }
        }
        enemy_ai_update(gs);
        return;
    ...
    while (mv->progress >= 1.0f && mv->moving) {
        ...
        if (mv->move_index < path_len) {
            ...
            render_set_info_text(gs, "Move complete — press Enter to end turn");
            break;
        }
    }
