...
void render_prerender_coord_textures(GameState *gs) {
    ...
    for (int r = 0; r < gs->map_rows; r++) {
        for (int c = 0; c < gs->map_cols; c++) {
            ...
            if (!surf) continue;
            gs->coord_textures[r * gs->map_cols + c] =
                SDL_CreateTextureFromSurface(gs->renderer, surf);
            gs->coord_tex_w = surf->w;
            gs->coord_tex_h = surf->h;
...
void render_free_coord_textures(GameState *gs) {
    ...
    int n = gs->map_rows * gs->map_cols;
    for (int i = 0; i < n; i++)
        if (gs->coord_textures[i]) SDL_DestroyTexture(gs->coord_textures[i]);
    free(gs->coord_textures);
    gs->coord_textures = NULL;
}
...
int render_set_info_lines(GameState *gs, const char **lines, int nlines) {
    ...
    for (int i = 0; i < nlines; ++i) {
        ...
        if (!surfs[i]) {
            for (int j = 0; j < i; ++j) SDL_FreeSurface(surfs[j]);
            free(surfs); return 0;
        }
    ...
    if (!dest) {
        for (int i = 0; i < nlines; ++i) SDL_FreeSurface(surfs[i]);
        free(surfs); return 0;
    }
...
int render_set_info_text(GameState *gs, const char *text) {
    ...
    return render_set_info_lines(gs, lines, 1);
}
...
static void make_bar(char *buf, size_t bufsz, int cur, int max, int width) {
    ...
    buf[pos] = '\0';
    char num[32];
    snprintf(num, sizeof(num), " %d/%d", cur, max);
...
void render_show_sprite_info_ex(GameState *gs, Sprite *s, UnitRole role, Faction faction) {
    ...
    char l_header[128], l_sep[64];
    snprintf(l_header, sizeof(l_header), "%s  %s",
             s->name ? s->name : "???",
             (role < 4) ? role_tag[role] : "");
    snprintf(l_sep, sizeof(l_sep), "--------------------");
    char l_job[128], l_level[64], l_faction[64], l_pos[64];
    ...
    char hp_bar[64], mp_bar[64], l_hp[96], l_mp[96];
    ...
    for (int i = 0; i < MAX_EQUIP_SLOTS; i++) {
        ...
        if (eq->name)
            snprintf(l_eq[i], sizeof(l_eq[i]), "%s : %s (atk+%d def+%d)",
                     slot_name[i], eq->name, eq->atk, eq->def);
        else
            snprintf(l_eq[i], sizeof(l_eq[i]), "%s : (none)", slot_name[i]);
    }
    const char *lines[] = {
        l_header, l_sep,
        l_job, l_level, l_faction, l_pos, l_sep,
        l_hp, l_mp, l_status, l_sep,
        l_atk, l_def, l_spd, l_mov, l_sep,
        l_eq[0], l_eq[1], l_eq[2], l_eq[3], l_eq[4]
    };
...
void render_hex_terrain(SDL_Renderer *renderer, int cx, int cy, int radius, Terrain t) {
    SDL_Point pts[6];
    hex_compute_points(cx, cy, radius, pts);
    Uint8 r, g, b, a;
    terrain_color(t, &r, &g, &b, &a);
    SDL_SetRenderDrawColor(renderer, r, g, b, a);
    fill_polygon(renderer, pts, 6);
    SDL_SetRenderDrawColor(renderer,
        r > 40 ? r - 40 : 0, g > 40 ? g - 40 : 0, b > 40 ? b - 40 : 0, a);
...
static void render_unit_sprite(GameState *gs, SDL_Rect src, int cx, int cy) {
    double hex_w = gs->current_radius * 2.0;
    double hex_h = gs->current_radius * sqrt(3.0);
    double scale = (hex_w * 0.9 / src.w < hex_h * 0.9 / src.h)
                 ? (hex_w * 0.9 / src.w) : (hex_h * 0.9 / src.h);
    if (scale <= 0.0) scale = 1.0;
    int dw = (int)(src.w * scale + 0.5);
    int dh = (int)(src.h * scale + 0.5);
...
static void pass_path_overlay(GameState *gs) {
    ...
    for (int row = 0; row < gs->map_rows; row++) {
        for (int col = 0; col < gs->map_cols; col++) {
            ...
            SDL_SetRenderDrawColor(rdr, 0, 200, 200, 140);
            fill_polygon(rdr, pts, 6);
        }
    }
}
...
static void pass_selection(GameState *gs) {
    ...
    int cx, cy;
    hex_center(gs->selected_row, gs->selected_col, gs->current_radius,
...
static void pass_neighbors(GameState *gs) {
    ...
    int rad = gs->current_radius;
    int nbr_r[6], nbr_c[6];
    int nc = get_neighbors(gs->hover_row, gs->hover_col,
                           gs->map_rows, gs->map_cols, nbr_r, nbr_c);
    SDL_Point pts[6];
    SDL_SetRenderDrawBlendMode(rdr, SDL_BLENDMODE_BLEND);
    for (int i = 0; i < nc; ++i) {
        int cx, cy;
        hex_center(nbr_r[i], nbr_c[i], rad, gs->camera.x, gs->camera.y, &cx, &cy);
        hex_compute_points(cx, cy, rad - 1, pts);
        SDL_SetRenderDrawColor(rdr, 255, 220, 0, 120);
        fill_polygon(rdr, pts, 6);
        SDL_SetRenderDrawColor(rdr, 255, 220, 0, 200);
        SDL_RenderDrawLines(rdr, pts, 6);
        SDL_RenderDrawLine(rdr, pts[5].x, pts[5].y, pts[0].x, pts[0].y);
    }
    int hcx, hcy;
    hex_center(gs->hover_row, gs->hover_col, rad, gs->camera.x, gs->camera.y, &hcx, &hcy);
    hex_compute_points(hcx, hcy, rad - 1, pts);
    SDL_SetRenderDrawColor(rdr, 255, 255, 255, 160);
    SDL_RenderDrawLines(rdr, pts, 6);
    SDL_RenderDrawLine(rdr, pts[5].x, pts[5].y, pts[0].x, pts[0].y);
}
...
static void pass_path_line(GameState *gs) {
    ...
    for (int i = 0; i < path_len; ++i) {
        int pr = path_nodes[i] / gs->map_cols;
        int pc = path_nodes[i] % gs->map_cols;
        hex_center(pr, pc, gs->current_radius,
                   gs->camera.x, gs->camera.y, &pline[i].x, &pline[i].y);
    }
    SDL_SetRenderDrawBlendMode(gs->renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(gs->renderer, 255, 80, 80, 220);
    SDL_RenderDrawLines(gs->renderer, pline, path_len);
    SDL_SetRenderDrawColor(gs->renderer, 255, 255, 255, 255);
    SDL_Rect rs = { pline[0].x - 4,           pline[0].y - 4,           8, 8 };
    SDL_Rect re = { pline[path_len-1].x - 4,  pline[path_len-1].y - 4,  8, 8 };
    SDL_RenderFillRect(gs->renderer, &rs);
    SDL_RenderFillRect(gs->renderer, &re);
    free(pline);
}
...
static void pass_units(GameState *gs) {
    ...
    for (int i = 0; i < gs->units.count; i++) {
        ...
        if (mv->moving && mv->unit_idx == i && mv->from_r >= 0 && mv->to_r >= 0) {
            ...
            cx = (int)(fx + (tx - fx) * t + 0.5f);
            cy = (int)(fy + (ty - fy) * t + 0.5f);
        } else {
            hex_center(u->sprite->x, u->sprite->y, rad,
                       gs->camera.x, gs->camera.y, &cx, &cy);
        }
        SDL_Rect src;
        if (u->role == UNIT_ROLE_PLAYER) {
            if (mv->moving && mv->unit_idx == i
                    && gs->atlas.run_frames > 0 && gs->atlas.run_w > 0) {
                src.x = gs->atlas.run_src.x + mv->run_frame * gs->atlas.run_w;
                src.y = gs->atlas.run_src.y;
                src.w = gs->atlas.run_w;
                src.h = gs->atlas.run_h;
            } else {
                src = gs->atlas.idle_src;
            }
        } else {
            src = (gs->atlas.role_src[u->role].w > 0)
                ? gs->atlas.role_src[u->role]
                : gs->atlas.idle_src;
        }
        if (!u->sprite->hp) {
            SDL_SetTextureColorMod(gs->atlas.tex, 200, 60, 60);
        } else if (u->faction == FACTION_ENEMY) {
            SDL_SetTextureColorMod(gs->atlas.tex, 255, 180, 180);
        } else if (u->role == UNIT_ROLE_PLAYER &&
                   gs->turn.player_moved && gs->turn.player_attacked) {
            SDL_SetTextureColorMod(gs->atlas.tex, 110, 110, 110);
        } else {
...
static void pass_ui(GameState *gs) {
    ...
    SDL_SetRenderDrawBlendMode(gs->renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(gs->renderer, 0, 0, 0, 160);
    SDL_RenderFillRect(gs->renderer, &bg);
...
void render_action_menu(GameState *gs) {
    ...
    for (int i = 0; i < MENU_ITEM_COUNT; i++) {
        SDL_Rect item = {
            m->anchor_x,
            m->anchor_y + i * m->item_h,
            m->item_w,
            m->item_h
        };
        int disabled = m->disabled[i];
        if (disabled)
            SDL_SetRenderDrawColor(rdr, 40, 40, 40, 180);
        else if (i == m->hovered)
            SDL_SetRenderDrawColor(rdr, 255, 220, 60, 230);
        else
            SDL_SetRenderDrawColor(rdr, 30, 30, 50, 210);
        SDL_RenderFillRect(rdr, &item);
        if (disabled)
            SDL_SetRenderDrawColor(rdr, 80, 80, 80, 180);
        else
            SDL_SetRenderDrawColor(rdr, 160, 160, 200, 255);
        SDL_RenderDrawRect(rdr, &item);
        SDL_Color col;
        if (disabled)
            col = (SDL_Color){100, 100, 100, 200};
        else if (i == m->hovered)
            col = (SDL_Color){20, 20, 20, 255};
        else
